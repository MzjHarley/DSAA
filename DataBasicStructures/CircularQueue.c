 ...
 * 链式队列较简单，接下来我们谈一下静态队列
 * 1.静态队列一般为什么必须为循环队列？ 传统数组实现队列会造成空间浪费；
 * 2.循环队列需要几个参数来确定？
 * 两个参数front和rear；front永远指向队列的第一个元素，rear永远指向最后一个有效元素的下一个位置.
 * 3.front和rear这两个参数在不同场合的不同含义:
 * 队列初始化：front和rear值都为0；
 * 队列为空：front和rear值相等但不一定为0；
 * 队列非空：front为第一个元素，rear为最后一个有效元素的下一个位置；
 * 4.循环队列入队伪算法：将值存入rear所代表的位置；rear=（rear+1）%数组长度；
 * 5.循环队列出队伪算法：front=（front+1）%数组长度；
 * 6.如何判断循环队列是否为满？
 * 要么多设置一个参数，该参数名为长度，但频繁更新较麻烦；要么存n个元素，找n+1个空间，空间浪费率为1/n+1，n越大，浪费率越低；这种方式front与rear相邻时为满；此时（rear+1）%数组长度=front；
 */
#include<stdio.h>
#include<stdlib.h>
#include<stdbool.h>
